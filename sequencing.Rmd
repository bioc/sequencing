...
```{r}
  dataDir <- system.file("extdata", package="pasillaBamSubset")
...
```{r}
library(ShortRead)
fls <- list.files(fastqDir, "fa$", full=TRUE) # was fastq
...
browseURL(report(qa)) # only if interactive() ?
  ```
...
sample. `Biostrings::matchPDict` (in some circumstances) and the
...
save(tt, file=file.path(dataDir, "tt.rda")) # save to outdir?
```
